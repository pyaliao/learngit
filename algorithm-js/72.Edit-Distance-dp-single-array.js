...
const minDistance = function (word1, word2) {
  ...
  // dp[j]代表word1前i个字符组成的字符串与word2前j个字符组成的字符串之间的最短编辑距离
  // 每次循环时，更新dp[j]，dp[j]代表了外层循环的每一层
  const m = word1.length
  const n = word2.length
  const dp = new Array(n + 1).fill()
  // 处理边界条件(对第一层相应的操作数进行填充)
  for (let j = 0; j <= n; j++) {
    dp[j] = j
  }
  for (let i = 1; i <= m; i++) {
    let leftTop = dp[0] // 每一行第一列对应的左上角的值即表格中的dp[i - 1][j - 1]
    dp[0] = i
    for (let j = 1; j <= n; j++) {
      // 此处的i,j代表的是第i个或者第j个数，因此在取值时i及j下标得减一
      // word1[i - 1]  word2[j - 1]
      const top = dp[j] // dp[i - 1][j]
      const left = dp[j - 1] // dp[i][j - 1]
      // 此处是i-1和j-1，因为数组下标从0开始，因此word1第i个字符与Word2第j个字符相等判断如下
      if (word1[i - 1] === word2[j - 1]) {
        dp[j] = leftTop
      } else {
        dp[j] = Math.min(leftTop, left, top) + 1
      }
      leftTop = top // 更新leftTop
    }
  }
  return dp[n]
}
